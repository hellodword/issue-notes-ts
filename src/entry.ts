...
async function deletePost(args: {
  ...
  console.log('deletePost', 'getPathSha', result);
  ...
  await args.rest.repos
    .deleteFile({
      ...
      repo: args.repo,
      branch: BRANCH_NAME,
      path: `${BASE_PATH_POSTS_NORMAL}/${args.issueNumber}/${result.name}`,
      message: `delete ${result.name} via github-actions${'\n\n'}${buildRawLink(
        args,
      )}`,
      sha: result.sha,
    })
    .then((response) => {
      console.log('deletePost', response.status);
    });
  return;
}
...
async function createPost(args: {
  ...
  if (result.img) {
    for (let i = 0; i < result.img.length; i++) {
      if (!result.img[i].link || result.img[i].link === '') {
        continue;
      }
  ...
  if (result.code) {
    for (let i = 0; i < result.code.length; i++) {
      if (!result.code[i].link || result.code[i].link === '') {
        continue;
      }
      // 优先用传参
